...
import hashlib 
import time # Necessário para os atrasos do WhatsApp
...
# ====================================================================
# 🌐 3. FUNÇÕES DE CONEXÃO E ENVIO (GSpread & Telegram API)
# ====================================================================
    ...
    try:
        ...
        if 'lista' in df.columns and 'nome' in df.columns and id_col in df.columns:
            for index, row in df.iterrows():
                nome_lista = str(row['lista']).strip(); destinatario_id = str(row[id_col]).strip()
...
def get_whatsapp_driver():
    ...
    # Adicione este argumento para o primeiro login (precisa da GUI)
    ...
    # Adicione logs para debug no servidor
...
def enviar_mensagem_whatsapp_api(driver, numero_destinatario, mensagem_processada):
    """Executa o envio via automação do WhatsApp Web (Selenium)."""
    
    # WhatsApp Web espera o número no formato internacional sem +
    ...
    try:
        driver.get(url)
        # 🛑 É CRÍTICO! Dê tempo para a página carregar e o chat abrir.
        time.sleep(5) 
        
        # Aqui, a lógica de envio (By.XPATH do botão) deveria ser implementada.
        # Estamos simplificando, confiando que o URL abre o chat e o usuário envia manualmente.
...
def processar_disparo(canal, listas_selecionadas, mensagem_original, uploaded_file, listas_dados):
    ...
    total_enviados = 0; erros = []; progress_bar = st.progress(0, text="Preparando envio...")
        ...
        with st.spinner(f'Iniciando envio {canal} para {len(destinatarios)} destinatários...'):
            for i, dest in enumerate(destinatarios):
                ...
                if canal == 'Telegram':
                    ...
                    # ⚠️ O driver de WhatsApp deve ser passado e usado
                    sucesso, resultado = enviar_mensagem_whatsapp_api(driver, chat_id, mensagem_processada)
...
# -------------------------------------------------------------
# RESTANTE DO CÓDIGO (LOGIN, UI) MANTIDO
# -------------------------------------------------------------
...
def app_ui():
    ...
    st.sidebar.markdown(f"Usuário: **{st.session_state['username']}**"); logout_button()
    ...
    if "Erro de Conexão" in listas_telegram_data: st.error("Falha ao carregar a lista do Telegram. Verifique as credenciais."); return 
    if "Erro de Colunas" in listas_telegram_data: st.error("Erro fatal: Colunas da lista TELEGRAM estão incorretas. Verifique 'lista', 'nome', 'ids'."); return 
    
    if "Erro de Conexão" in listas_whatsapp_data: st.warning("⚠️ Falha na conexão para WhatsApp. A funcionalidade WhatsApp será limitada.")
    ...
    with tab_telegram:
        ...
        for nome_lista in imediato_listas_selecionadas: imediato_ids_para_disparo.update([d['id'] for d in listas_telegram_data.get(nome_lista, [])])
    ...
    with tab_whatsapp:
        ...
        st.warning("⚠️ RISCO DE BLOQUEIO: O número deve estar logado no WhatsApp Web na sessão salva. Feche este app ao terminar.")
        ...
        if st.button("🚀 Disparar WhatsApp Agora", key="btn_whatsapp", type="primary"):
            ...
            processar_disparo_whatsapp(whatsapp_listas_selecionadas, whatsapp_mensagem, whatsapp_uploaded_file, listas_whatsapp_data)
